...
class BrgyOfficialController extends Controller
{
    ...
    public function barangayPOST(Request $request){
        ...
        if(!(($position->position ?? '') == 'Punong Barangay' || ($position->position ?? '') == 'Barangay Secretary' || ($position->position ?? '') == 'Barangay Record Keeper' || ($position->position ?? '') == 'Registry of Barangay Inhabitants' || ($position->position ?? '') == 'Messenger' || ($position->position ?? '') == 'Utility' || ($position->position ?? '') == 'SK Chairman' || ($position->position ?? '') == 'Barangay Treasurer'  )){
        ...
        if($barangay <= 15){
            ...
            return response()->json(['status'=>5,'failed'=>'No more than 15 member.']);
        }

    }else{


        return response()->json(['status'=>5,'duplicate'=>'A Unique Position is already Existed or member Exceed 15.']);
